...
def make_figures(s: dict, rows: list[dict]) -> dict[str, str]:
    ...
    plt.figure(figsize=(10, 4.3)); x=np.arange(len(models)); w=.36
    plt.bar(x-w/2,v1,w,label="v1 F1",color=BLUE);plt.bar(x+w/2,v2,w,label="v2 F1",color=CYAN)
    ax=plt.gca(); ax2=ax.twinx(); ax2.plot(x,errs,"o--",color=RED,label="오류 수");ax2.set_ylabel("호출 오류 수",color=RED)
    plt.xticks(x,[m.replace(":","\n") for m in models],fontsize=7.5);ax.set_ylim(.8,1.01);ax.set_ylabel("ITT F1")
    lines, labs = ax.get_legend_handles_labels(); lines2,labs2=ax2.get_legend_handles_labels();ax.legend(lines+lines2,labs+labs2,loc="lower left",ncol=3)
    plt.title("모델별 강건성 및 운영 오류")
    ...
    plt.figure(figsize=(8.4,2.6));plt.axis("off")
    plt.table(cellText=[[f"TN {cm[0,0]:,}\nFP {cm[0,1]:,}",f"TN {cm[0,2]:,}\nFP {cm[0,3]:,}"],[f"FN {cm[1,0]:,}\nTP {cm[1,1]:,}",f"FN {cm[1,2]:,}\nTP {cm[1,3]:,}"]],
              rowLabels=["정상", "공격"],colLabels=["ToolProof v1","ToolProof v2"],cellLoc="center",loc="center",colWidths=[.35,.35])
...
def report(s: dict, rows: list[dict], figs: dict[str,str]) -> None:
    ...
    story=[P("MCP ToolProof 확장 실험 결과 보고서","TitleK"),P("10개 모델 · 공격 방법 변화 · 계약 적대 테스트 · TCP/SQLite · 공식 MCP SDK","H2K"),
           table([["검증 모집단","실행/결과"],["LLM tool-calling agent loop",f"{s['runs']:,} 계획, {s['valid']:,} 유효, {s['errors']:,} 오류"],["적대적 계약 단위 테스트","12,000회"],["별도 프로세스 TCP + SQLite observer","2,800회"],["공식 MCP Python SDK 1.29.0 stdio","1,400회"],["전체 실행량",f"{total_layers:,}회(층별 모집단은 성능 산출 시 분리)"]],[63*mm,105*mm]),Spacer(1,3*mm),Image(figs["layers"],width=166*mm,height=63*mm),
           ...
           PageBreak(),P("2. 모든 방어의 정량 결과","H1K"),Image(figs["defenses"],width=174*mm,height=71*mm),Image(figs["cm"],width=165*mm,height=49*mm),
           ...
           PageBreak(),P("3. 모델·공격 방법 변화에 대한 강건성","H1K"),Image(figs["models"],width=174*mm,height=75*mm),Image(figs["attacks"],width=174*mm,height=52*mm),
           ...
           PageBreak(),P("4. 계약 적대 테스트와 실패 분석","H1K"),
           ...
           PageBreak(),P("6. 위협·실패·확장 가능성","H1K"),
           ...
           PageBreak(),P("7. 논문용 핵심 결과와 제출 체크리스트","H1K"),
...
def paper(s: dict, figs: dict[str,str]) -> None:
    ...
    def col(items,x,y):
        for item in items:y=draw_item(c,item,x,y,cw)
        return y
